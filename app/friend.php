...
// Api for send Friend, cancel Friend, UnFriend and accept Friend
$app->post('/performaction', function ($request,  $response,  $args) {
	...
	if ($operationType == 1) {
		...
		// ( is_bool($deletion1) || is_bool($deletion2) )
...
/* 			$stmt = $pdo->prepare("DELETE FROM `notifications` WHERE `notificationTo`=:notificationTo AND `notificationFrom`=:notificationFrom
		...
		if (is_bool($result)) {
			...
			// sending notification to the device 
